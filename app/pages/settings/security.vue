...
async function toggleTwoFactor(enabled: boolean) {
  if (enabled) {
    try {
      const setupData = await settingsStore.setupTwoFactor()
      if (!setupData) {
        feedback.value = { tone: 'error', text: error.value || 'Failed to initialize 2FA setup.' }
        return
      }
      ...
      const code = prompt(
        ...
        `2. Add a new account manually with the following Secret Key:\n\n` +
        `   ${setupData.secret}\n\n` +
        `3. Enter the 6-digit code generated by your app below:`
      )

      if (!code) {
        feedback.value = { tone: 'warning', text: '2FA setup cancelled.' }
        return
      }

      const verifyData = await settingsStore.verifyTwoFactor(code)
      if (verifyData && verifyData.backupCodes) {
        const codesList = verifyData.backupCodes.join('\n')
        alert(
          `2FA enabled successfully!\n\n` +
          `IMPORTANT: Save these recovery backup codes in a safe place. ` +
          `If you lose your authenticator device, you can use these codes to log in. ` +
          `They will not be shown again:\n\n` +
          codesList
        )
        feedback.value = { tone: 'success', text: 'Two-factor authentication enabled.' }
        await settingsStore.fetchSecurityData()
      } else {
        feedback.value = { tone: 'error', text: error.value || 'Verification failed. Incorrect code.' }
      }
    } catch (err: any) {
      feedback.value = { tone: 'error', text: err.message || 'Failed to complete 2FA setup.' }
    }
  } else {
    const confirmDisable = confirm('Are you sure you want to disable Two-Factor Authentication?')
    if (!confirmDisable) return

    const success = await settingsStore.disableTwoFactor()
    if (success) {
      feedback.value = { tone: 'success', text: 'Two-factor authentication disabled.' }
      await settingsStore.fetchSecurityData()
    } else {
      feedback.value = { tone: 'error', text: error.value || 'Failed to disable 2FA.' }
    }
  }
}

async function handleBackupCodes() {
  const confirmGen = confirm('For security, backup codes are stored hashed and cannot be viewed. Would you like to regenerate 10 new backup codes?')
  if (!confirmGen) return
  const api = useApi()
  const response = await api.post('/security/backup-codes/regenerate')
  if (response.success && response.data?.backupCodes) {
    alert(`New backup codes generated. Please copy and save them in a safe place. They will not be shown again:\n\n${response.data.backupCodes.join('\n')}`)
    await settingsStore.fetchSecurityData()
  } else {
    alert(response.message || 'Failed to regenerate backup codes.')
  }
}
...
function formatLocation(loc: string, ip?: string) {
  const isFakeLocation = (l: string) => {
    const val = l.toLowerCase()
    return val.includes('rabat') || val.includes('casablanca') || val.includes('marrakech') || val.includes('morocco')
  }

  if (loc && loc.trim() !== '' && loc.toLowerCase() !== 'unknown' && loc !== 'Local' && loc !== '::1' && loc !== '127.0.0.1' && !isFakeLocation(loc)) {
    return loc
  }
  if (ip && ip.trim() !== '' && ip.toLowerCase() !== 'unknown') {
    return `IP: ${ip}`
  }
  return 'Unknown location'
}

function formatSessionSubtitle(loc: string, ip: string) {
  const isFakeLocation = (l: string) => {
    const val = l.toLowerCase()
    return val.includes('rabat') || val.includes('casablanca') || val.includes('marrakech') || val.includes('morocco')
  }

  const hasRealLoc = loc && loc.trim() !== '' && loc.toLowerCase() !== 'unknown' && loc !== 'Local' && loc !== '::1' && loc !== '127.0.0.1' && !isFakeLocation(loc)
  const hasRealIp = ip && ip.trim() !== '' && ip.toLowerCase() !== 'unknown'

  if (hasRealLoc) {
    return hasRealIp ? `${loc} | IP: ${ip}` : loc
  }
  if (hasRealIp) {
    return `IP: ${ip}`
  }
...
function formatLastActivity(dateString: string, isCurrent: boolean) {
...
function formatAction(action: string) {
  ...
  const map: Record<string, string> = {
    ...
    security_update: 'Security Preferences Updated',
    register: 'Registration'
  }
  return map[action] || action.split('_').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')
}
</script>
      ...
      <div class="space-y-6">
        ...
        <div class="grid gap-[13px] lg:grid-cols-2">
          ...
          >
            ...
            <div v-else class="sec-card__rows">
              <div
                v-for="session in (showAllSessions ? sessions : sessions.slice(0, 3))"
                :key="session.id"
                ...
                <div class="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div class="sec-row__body !gap-2">
                    ...
                    <span
                      ...
                      :title="formatSessionSubtitle(session.location, session.ipAddress)"
                    >
                      {{ formatSessionSubtitle(session.location, session.ipAddress) }}
                    </span>
                  </div>
                </div>
                <div class="flex items-center gap-3 flex-shrink-0">
                  <span class="sec-row__sub">{{ formatLastActivity(session.lastActivity, session.current) }}</span>
                  <button
                    v-if="!session.current"
                    type="button"
                    class="sec-card__action-link !text-[var(--color-danger)]"
          ...
          <div class="sec-card">
            ...
            <div v-else class="sec-card__rows">
              <div
                v-for="entry in (showAllHistory ? loginHistory : loginHistory.slice(0, 3))"
                :key="entry.id"
                class="sec-row sec-row--spaced"
              >
                <div class="sec-row__body">
                  <div class="sec-row__title-line">
                    <span class="sec-row__label">{{ formatAction(entry.action) }}</span>
                    <span class="sec-row__sub sec-row__sub--truncate">
                      {{ entry.timeLabel }} • {{ formatLocation(entry.location, entry.ipAddress) }}
                    </span>
                  </div>
                </div>
            ...
            >
              ...
              >
                {{ showAllHistory ? 'Show less' : 'View all histories' }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-6">
        <section class="section-card">
          ...
          <div class="mt-6 space-y-3">
            ...
            >
              <div class="flex items-center gap-3">
                <div :class="['icon-box', check.completed ? 'icon-box-success' : 'icon-box-warning']">
                  <UIcon
                    :name="check.completed ? 'i-lucide-check' : 'i-lucide-alert-triangle'"
                    class="size-4"
                  />
                </div>
