...
<template>
  <section class="max-w-[1400px] mx-auto pb-10">
    ...
    <div
      v-else
      class="grid grid-cols-1 xl:grid-cols-[280px_1fr_280px] gap-6"
    >
      <!-- Sidebar course list -->
      <div class="flex flex-col gap-2">
        <input
          ...
          class="w-full h-[36px] px-3 text-[13px] focus:outline-none mb-1"
        ...
        >
          <div
            ...
            style="border-radius:8px;background:color-mix(in srgb,var(--color-primary) 12%,transparent);color:var(--color-primary)"
          >
            <UIcon
          ...
          <div class="min-w-0 flex-1">
            <p
              ...
              :style="selected?._id===c._id?'color:var(--color-primary)':'color:var(--color-text)'"
            >
              {{ c.title }}
            </p>
            <p
              ...
              style="color:var(--color-text-muted)"
            >
              {{ c.teacher || 'Prof.' }} • {{ c.semester || 'S2' }}
            </p>
          </div>
          <span
            class="text-[11px] font-bold px-1.5 py-0.5"
            style="border-radius:5px;background:color-mix(in srgb,var(--color-success) 15%,transparent);color:var(--color-success)"
          >{{ c.progress||75 }}%</span>
        </button>
        <div
        ...
        <div
          class="mt-1 flex flex-col items-center gap-1.5 p-4 cursor-pointer transition"
          style="border-radius:12px;border:2px dashed var(--color-border);color:var(--color-text-muted)"
          @click="uploadRef?.click()"
        >
          <UIcon
          ...
          <p class="text-[11px] font-semibold text-center">
            Drag & drop or click to upload<br><span class="font-normal opacity-70">PDF, DOCX, XLSX up to 50MB</span>
          </p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <p
            ...
            style="color:var(--color-text-muted)"
          >
            {{ selected.teacher||'Prof. Smith' }} • {{ selected.semester||'Semester 2 • 2024' }}
          </p>
          <p
            class="text-[13px] leading-relaxed mb-5"
            style="color:var(--color-text-soft)"
          >
            {{ selected.description||'This course covers essential topics including core concepts, theories, and practical applications. Use the resources and summaries to stay on top of your studies.' }}
          </p>
          <div class="mb-5">
            ...
            >
              <div
                class="h-full"
                :style="`width:${selected.progress||75}%;border-radius:99px;background:var(--color-success)`"
              />
            </div>
          </div>
          <div class="grid grid-cols-4 gap-3">
            <div
              v-for="s in [{ l: 'Files', i: 'i-lucide-files', v: selected.filesCount||24 }, { l: 'Summaries', i: 'i-lucide-book-marked', v: 12 }, { l: 'Exercises', i: 'i-lucide-pen-line', v: 18 }, { l: 'Plans', i: 'i-lucide-calendar-check', v: 5 }]"
              :key="s.l"
              class="flex flex-col items-center p-3 text-center"
              style="border-radius:12px;background:color-mix(in srgb,var(--color-primary) 6%,transparent);border:1px solid var(--color-border)"
            >
              <UIcon
                :name="s.i"
                class="size-4 mb-1"
        ...
        <div
          class="p-5"
          style="background:color-mix(in srgb,var(--color-primary) 5%,var(--color-surface));border-radius:16px;border:1px solid var(--color-border)"
        >
          ...
          <ul class="space-y-2">
            <li
              v-for="pt in ['Strong emphasis on core theory and applications', 'Includes practical exercises and problem sets', 'Regular assignments build analytical foundation', 'Exam-focused revision materials available']"
              :key="pt"
      ...
      >
        ...
        <div class="space-y-3">
          ...
          >
            <div
              class="flex size-[36px] flex-shrink-0 items-center justify-center"
              :style="`border-radius:10px;background:${fileColor(f.type)}`"
            >
              <UIcon
            ...
            <div class="min-w-0 flex-1">
              ...
              <p
                ...
                style="color:var(--color-text-muted)"
              >
                {{ String(f.type).toUpperCase() }} • {{ (f.size/1024/1024).toFixed(1) }}MB • {{ ago(f.createdAt) }}
              </p>
            </div>
          </div>
        </div>
