...
<template>
  <section
    ...
    class="flex h-full min-h-[50vh] items-center justify-center"
  >
    <UIcon
  ...
  <section
    v-else
    class="max-w-[1400px] mx-auto pb-10"
  >
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[1fr_360px] gap-6">
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <!-- Classes Today -->
          <div
            class="rounded-[12px] p-4 shadow-[var(--shadow-card)] border border-[var(--color-border)]"
          ...
          >
            ...
            <p
              class="mt-3 text-[24px] font-bold leading-none"
            ...
            <p
              ...
              :style="nextClass ? 'color:var(--color-success)' : 'color:var(--color-text-muted)'"
            >
          ...
          >
            ...
            <p
              class="mt-2 text-[11px] font-medium"
              :style="highPriorityTasksCount > 0 ? 'color:var(--color-danger)' : 'color:var(--color-text-muted)'"
            >
        ...
        <div
          class="rounded-[12px] shadow-[var(--shadow-card)] border border-[var(--color-border)]"
          style="background:var(--color-surface)"
        >
          <!-- Card header -->
          <div class="flex items-center justify-between px-6 pt-5 pb-4">
            <div class="flex items-center gap-2">
              <UIcon
                ...
                class="size-5 text-slate-700 dark:text-white"
              />
              <h2 class="text-[16px] font-bold text-slate-900 dark:text-white">
                Today's Plan
              </h2>
            </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr]">
            ...
            <div class="p-6">
              <div class="relative pl-[52px]">
                <div
                  class="absolute left-[44px] top-4 bottom-4 w-[2px]"
                  style="background:var(--color-border)"
                />
                <div
                  v-if="todaysPlan.length === 0"
                  class="text-[16px] text-slate-500 py-4 text-center"
                >
                  No events scheduled for today.
                </div>
                <div class="space-y-4 relative z-10">
                  <div
                    v-for="plan in todaysPlan"
                    :key="plan._id"
                    class="relative flex items-center gap-4"
                  >
                    <div
                      class="absolute -left-[64px] w-[46px] text-right text-[12px] font-bold pt-0.5"
                      style="color:var(--color-text-soft);background:var(--color-surface)"
                    >
                      {{ formatTime(plan.start) }}
                    </div>
                    <div
                      class="absolute -left-[14px] flex items-center justify-center py-1"
                      style="background:var(--color-surface)"
                    >
                      <div
                        class="size-[10px] rounded-full border-2 border-white dark:border-slate-800"
                        :class="{
                    ...
                    <div
                      class="flex-1 p-3 flex items-center gap-4 transition-all cursor-pointer"
                    ...
                    >
                      <div
                        class="flex size-[36px] flex-shrink-0 items-center justify-center"
                        :style="[
                          'border-radius:8px',
                      ...
                      >
                        <UIcon
                          v-if="plan.type === 'study_session'"
                          name="i-lucide-sparkles"
                          class="size-4"
                        />
                        <UIcon
                          v-else-if="plan.type === 'course'"
                          name="i-lucide-book-open"
                          class="size-4"
                        />
                        <UIcon
                          v-else-if="plan.type === 'td' || plan.type === 'tp'"
                          name="i-lucide-calculator"
                          class="size-4"
                        />
                        <UIcon
                          v-else-if="plan.type === 'exam'"
                      ...
                      <div class="min-w-0 flex-1">
                        <p
                          ...
                          :style="plan.type==='study_session' ? 'color:var(--color-primary)' : 'color:var(--color-text)'"
                        >
                          {{ plan.title }}
                        </p>
                        <p
                          class="text-[11px] font-medium mt-0.5 truncate"
                          :style="plan.type==='study_session' ? 'color:var(--color-primary);opacity:0.7' : 'color:var(--color-text-muted)'"
                        >
                          {{ plan.location || (plan.type === 'study_session' ? 'AI Suggested' : plan.type.toUpperCase()) }}
                        </p>
                      </div>
                      <div
                        v-if="plan.type === 'study_session'"
                        class="size-6 rounded-full border border-indigo-200 flex items-center justify-center mr-1"
                      >
                        <UIcon
                          name="i-lucide-plus"
            ...
            <!-- Right: AI Suggestion -->
            <div
              class="p-5 m-4 flex flex-col"
              style="background:color-mix(in srgb,var(--color-primary) 5%,var(--color-surface));border-radius:12px;border:1px solid var(--color-border)"
            >
              ...
              <p
                class="text-[13px] leading-relaxed font-medium mb-4"
                style="color:var(--color-text-muted)"
              >
              ...
              >
                <UIcon
                  name="i-lucide-loader-2"
                  class="size-6 animate-spin text-violet-500"
                />
              </div>
              <div
                v-else-if="aiPlan"
              ...
              >
                <div
                  v-for="(item, idx) in aiPlan.plan"
                  :key="idx"
                  class="flex items-center gap-3 rounded-[10px] p-3 border border-[var(--color-border)] shadow-[var(--shadow-sm)]"
                  style="background:var(--color-surface)"
                >
                  <UIcon
                    name="i-lucide-check-circle-2"
                    class="size-5 text-emerald-500 flex-shrink-0"
                  />
                  <div class="min-w-0 flex-1">
                  ...
                  <UIcon
                    ...
                    class="size-4 text-slate-300 flex-shrink-0"
                  />
                </div>
              </div>
              <div
              ...
              <UButton
                ...
                color="primary"
                variant="soft"
                class="mt-4 font-bold text-[16px] h-10 rounded-[8px] bg-violet-50 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 hover:bg-violet-100 transition"
        ...
        <div class="bg-gradient-to-r from-[#4f46e5] to-[#6366f1] rounded-[12px] p-8 text-white shadow-[0_8px_30px_-10px_rgba(79,70,229,0.4)] flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <p class="text-indigo-100 text-[12px] font-medium uppercase tracking-wider mb-2">
              Focus Plan
            </p>
            <h3 class="text-[26px] font-bold leading-[1.2] mb-2 tracking-tight">
              Stay consistent,<br>achieve your goals.
            </h3>
            <p class="text-indigo-100/90 text-[13px] font-medium">
          ...
          <div class="flex items-center gap-10 bg-white/10 rounded-[10px] p-5 backdrop-blur-md border border-white/10">
            <div class="text-center">
              <p class="text-[12px] text-indigo-100 mb-3 font-semibold">
                This Week Progress
              </p>
              ...
              <p class="text-[10px] text-indigo-100 mt-3 font-bold uppercase tracking-wider">
                Great progress!
              </p>
            </div>
            <div class="flex items-end gap-3 h-[72px]">
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white/30 h-8" /><span class="text-[9px] font-bold text-indigo-100 uppercase">Mon</span>
              </div>
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white/30 h-10" /><span class="text-[9px] font-bold text-indigo-100 uppercase">Tue</span>
              </div>
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white h-16 shadow-[0_0_12px_rgba(255,255,255,0.6)]" /><span class="text-[9px] font-bold text-white uppercase">Wed</span>
              </div>
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white/30 h-9" /><span class="text-[9px] font-bold text-indigo-100 uppercase">Thu</span>
              </div>
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white/30 h-12" /><span class="text-[9px] font-bold text-indigo-100 uppercase">Fri</span>
              </div>
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white/30 h-6" /><span class="text-[9px] font-bold text-indigo-100 uppercase">Sat</span>
              </div>
              <div class="flex flex-col items-center gap-1.5">
                <div class="w-2 rounded-full bg-white/30 h-8" /><span class="text-[9px] font-bold text-indigo-100 uppercase">Sun</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div
          class="p-6"
          style="background:var(--color-surface);border-radius:16px;border:1px solid var(--color-border);box-shadow:var(--shadow-card)"
        >
          <div class="flex items-center justify-between mb-5">
            <h3 class="font-bold text-[16px] text-slate-900 dark:text-white">
              Recent Files
            </h3>
            <NuxtLink
              to="/dashboard/files"
              class="text-[16px] font-bold text-indigo-600 hover:text-indigo-700"
            >View all</NuxtLink>
          </div>
          <div class="grid gap-4 md:grid-cols-3">
            ...
            <div
              v-for="file in files"
              :key="file._id"
              class="flex flex-col p-4 transition"
              style="border-radius:10px;border:1px solid var(--color-border);background:var(--color-surface);box-shadow:var(--shadow-sm)"
            >
              <div class="flex items-center gap-3 mb-4">
                <div
                  class="flex size-[42px] flex-shrink-0 items-center justify-center"
                  :style="[
                    'border-radius:12px',
                ...
                >
                  <UIcon
                    :name="file.type === 'pdf' ? 'i-lucide-file-text' : 'i-lucide-file'"
                    class="size-[20px]"
                  />
                </div>
                <p class="text-[16px] font-bold text-slate-900 dark:text-white truncate flex-1">
                  {{ file.originalName || file.fileName }}
                </p>
              </div>
              <p class="text-[16px] font-medium text-slate-500">
                {{ String(file.type).toUpperCase() }} • {{ (file.size / 1024 / 1024).toFixed(1) }} MB
              </p>
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-2">
              ...
              />
              <h2 class="text-[24px] font-bold text-slate-900 dark:text-white">
                AI Assistant
              </h2>
            </div>
          ...
          <div class="flex-1 space-y-3">
            <template v-if="chatReplies.length === 0">
              <div
                ...
                style="border-radius:8px;border:1px solid var(--color-border);background:var(--color-surface);box-shadow:var(--shadow-sm)"
                @click="sendChat('Summarize my course')"
              >
                <div
                  class="size-6 flex items-center justify-center"
                  style="border-radius:6px;background:color-mix(in srgb,var(--color-primary) 10%,transparent);color:var(--color-primary)"
                >
                  <UIcon
                ...
                <span
                  class="text-[12px] font-bold"
                  style="color:var(--color-text)"
                >Summarize my course</span>
              </div>

              <div
                class="flex items-center gap-3 p-3 cursor-pointer transition"
                style="border-radius:8px;border:1px solid var(--color-border);background:var(--color-surface);box-shadow:var(--shadow-sm)"
                @click="sendChat('Create a study plan')"
              >
                <div
                  class="size-6 flex items-center justify-center"
                  style="border-radius:6px;background:color-mix(in srgb,var(--color-primary) 10%,transparent);color:var(--color-primary)"
                ...
                <span
                  class="text-[12px] font-bold"
                  style="color:var(--color-text)"
                >Create a study plan</span>
              </div>

              <div
                class="flex items-center gap-3 p-3 cursor-pointer transition"
                style="border-radius:8px;border:1px solid var(--color-border);background:var(--color-surface);box-shadow:var(--shadow-sm)"
                @click="sendChat('Generate exercises')"
              >
                <div
                  class="size-6 flex items-center justify-center"
                  style="border-radius:6px;background:color-mix(in srgb,var(--color-primary) 10%,transparent);color:var(--color-primary)"
                ...
                <span
                  class="text-[12px] font-bold"
                  style="color:var(--color-text)"
                >Generate exercises</span>
              </div>

              <div
                class="flex items-center gap-3 p-3 cursor-pointer transition"
                style="border-radius:8px;border:1px solid var(--color-border);background:var(--color-surface);box-shadow:var(--shadow-sm)"
                @click="sendChat('What should I study today?')"
              >
                <div
                  class="size-6 flex items-center justify-center"
                  style="border-radius:6px;background:color-mix(in srgb,var(--color-primary) 10%,transparent);color:var(--color-primary)"
                ...
                <span
                  class="text-[12px] font-bold"
                  style="color:var(--color-text)"
                >What should I study today?</span>
              </div>
            </template>
            <div
              v-else
              class="space-y-4 max-h-56 overflow-y-auto pr-2 scrollbar-thin"
            >
              <div
                v-for="(msg, i) in chatReplies"
                :key="i"
                class="text-[13px] p-3 font-medium"
          ...
          <div class="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800 relative">
            <div class="relative flex items-center">
              ...
              >
              <button
                class="absolute right-1.5 size-[30px] flex items-center justify-center cursor-pointer transition disabled:opacity-50"
        ...
        >
          <div class="flex items-center justify-between mb-6">
            ...
            <NuxtLink
              ...
              class="text-[13px] font-bold text-indigo-600 hover:text-indigo-700"
            >View all</NuxtLink>
          </div>
          ...
          <div class="space-y-5">
            <div
              ...
              class="text-[13px] text-slate-500 py-2 font-medium"
            >
              No upcoming items.
            </div>
            <div
              v-for="(item, idx) in upcomingItems"
              :key="item.id"
              class="flex gap-4 items-center"
              :style="idx !== upcomingItems.length - 1 ? 'padding-bottom:20px;border-bottom:1px solid var(--color-border)' : ''"
            >
              <div
                class="w-[42px] text-center flex-shrink-0 flex flex-col justify-center pr-4"
              ...
              <div class="min-w-0 flex-1 pl-1">
                <p class="text-[16px] font-bold text-slate-900 dark:text-white truncate">
                  {{ item.title }}
                </p>
                <p class="text-[11px] font-medium text-slate-500 mt-1">
                  {{ formatDateShort(item.date) }} • {{ formatTime(item.date.toISOString()) || '10:00 AM' }}
                </p>
              </div>
              <span
                class="text-[10px] px-2.5 py-1 font-bold whitespace-nowrap tracking-wide uppercase"
                :class="getDaysUntilClass(item.date)"
        ...
        >
          ...
          <div class="space-y-4">
            ...
            >
              <div
                class="flex size-[42px] flex-shrink-0 items-center justify-center"
                style="border-radius:12px;background:color-mix(in srgb,var(--color-primary) 10%,transparent);color:var(--color-primary);box-shadow:var(--shadow-sm)"
              >
                <UIcon
                  name="i-lucide-book-open"
                  class="size-5"
              ...
              <div class="min-w-0 flex-1 relative pr-[38px]">
                ...
                <p class="text-[16px] font-medium text-slate-500 mt-1">
                  {{ course.teacher || 'Prof. Smith' }} • {{ course.semester || 'Fall 2024' }}
                </p>
