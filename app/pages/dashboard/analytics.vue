    ...
    <div class="flex items-center justify-end mb-6">
      <div class="flex items-center gap-2">
        <button
          ...
          :style="period==='week'?'border-radius:8px;background:var(--color-primary);color:#fff':'border-radius:8px;background:var(--color-surface);border:1px solid var(--color-border);color:var(--color-text)'"
          @click="period='week'"
        >
          This Week
        </button>
        <button
          class="px-3 py-1.5 text-[12px] font-bold transition"
          :style="period==='month'?'border-radius:8px;background:var(--color-primary);color:#fff':'border-radius:8px;background:var(--color-surface);border:1px solid var(--color-border);color:var(--color-text)'"
          @click="period='month'"
        >
          This Month
        </button>
    ...
    >
        ...
        >
          <div class="flex items-center gap-2 mb-3">
            <div
              ...
              :style="`border-radius:7px;background:color-mix(in srgb,var(--color-${s.c}) 12%,transparent);color:var(--color-${s.c})`"
            >
              <UIcon
                :name="s.i"
                class="size-3.5"
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
        <div class="flex flex-col gap-6">
          ...
          >
            <h3
              class="text-[15px] font-bold mb-5"
              style="color:var(--color-text)"
            >
              Study Hours by Week
            </h3>
            <div class="flex items-end gap-3 h-[120px]">
              ...
              >
                ...
                >{{ w.hours.toFixed(0) }}h</span>
                <div
                  class="w-full transition-all"
                  :style="`height:${Math.max(6, Math.min(100, w.hours/8*100))}px;border-radius:6px 6px 0 0;background:var(--color-primary);opacity:${w.hours>0?1:0.3}`"
                />
                <span
                  class="text-[10px] font-bold"
                  style="color:var(--color-text-muted)"
          ...
          >
            <div class="flex items-center justify-between mb-5">
              ...
              <p
                ...
                style="color:var(--color-text-muted)"
              >
                Based on tasks & exam performance
              </p>
            </div>
            <div
              v-if="!subjectPerformance.length"
              ...
              >
                <div class="flex items-center justify-between mb-1.5">
                  ...
                  <span
                    ...
                    :style="s.score>=80?'color:var(--color-success)':s.score>=60?'color:var(--color-warning)':'color:var(--color-danger)'"
                  >{{ s.score }}%</span>
                </div>
                <div
                ...
                >
                  <div
                    ...
                    :style="`width:${s.score}%;border-radius:99px;background:${s.score>=80?'var(--color-success)':s.score>=60?'var(--color-warning)':'var(--color-danger)'}`"
                  />
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div class="flex items-center gap-6">
              <div
                class="relative size-[100px] flex-shrink-0 flex items-center justify-center"
                style="border-radius:50%;background:conic-gradient(var(--color-primary) 0% {{productivityScore}}%, var(--color-border) {{productivityScore}}% 100%)"
              >
                <div
                  class="size-[80px] flex items-center justify-center"
                  style="border-radius:50%;background:var(--color-surface)"
                >
                  <span
              ...
              <div>
                <p
                  ...
                  style="color:var(--color-text-muted)"
                >
                  Based on tasks and study plans
                </p>
                <div class="mt-3 space-y-1.5">
                  <div class="flex items-center gap-2">
                    ...
                    /><span
                      class="text-[12px]"
                      style="color:var(--color-text)"
                    >Completed {{ tasks.filter(t => t.status==='completed').length }}/{{ tasks.length }} tasks</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <div
                      class="size-2.5 rounded-full"
        ...
        <div class="flex flex-col gap-5">
            ...
            <div class="space-y-3">
              ...
              >
                <UIcon
                  ...
                  :style="ins.type==='success'?'color:var(--color-success)':ins.type==='warning'?'color:var(--color-warning)':'color:var(--color-info)'"
                />
                <p
                  class="text-[12px]"
                  style="color:var(--color-text-soft)"
          ...
          >
            ...
            <div class="space-y-3">
              ...
              >
                <div
                  class="flex size-8 flex-shrink-0 items-center justify-center"
                  style="border-radius:8px;background:color-mix(in srgb,var(--color-primary) 10%,transparent);color:var(--color-primary)"
                >
                  <UIcon
                    :name="r.icon"
                    class="size-4"
          ...
          >
            ...
            <div class="grid grid-cols-7 gap-1">
              <div
                ...
                :key="i"
                class="aspect-square"
                :style="`border-radius:4px;background:${i%3===0?'var(--color-primary)':i%5===0?'color-mix(in srgb,var(--color-primary) 40%,transparent)':'var(--color-border)'}`"
              />
            </div>
            <p
